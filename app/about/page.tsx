...
function buildRegionStats() {
  const map = new Map<string, { offices: number; contracts: number; topMonthlyFee: number }>()
  for (const c of partnerCases) {
    const r = map.get(c.region) ?? { offices: 0, contracts: 0, topMonthlyFee: 0 }
    map.set(c.region, {
      offices:      r.offices + 1,
      contracts:    r.contracts + c.contracts,
      topMonthlyFee: Math.max(r.topMonthlyFee, c.monthlyFee),
    })
  }
  return REGION_ORDER.map(name => ({
    name,
    ...(map.get(name) ?? { offices: 0, contracts: 0, topMonthlyFee: 0 }),
  }))
}
  ...
  return (
    <div className="bg-white text-[#0a0a0a]">
      ...
      <header className="flex items-center justify-between px-6 py-5 md:px-10 md:py-7 border-b border-black/8">
        <Link
          href="/"
          className="font-inter font-black text-[#0a0a0a] text-sm tracking-[0.22em] uppercase select-none hover:opacity-60 transition-opacity"
        >
          BizplatForm
        </Link>
        ...
        <Link
          href="/diagnosis"
          className="md:hidden text-[#0a0a0a]/55 text-xs tracking-widest border border-black/20 rounded-full px-3 py-1.5 hover:text-[#0a0a0a]/80 transition-colors"
        >
          AI診断
        </Link>
      </header>
      ...
      <main>
        ...
        <section className="border-t border-black/8 bg-[#0a0a0a] text-white px-6 md:px-10 py-20 md:py-28">
          <div className="max-w-5xl mx-auto">
            <p className="text-white/30 text-[10px] tracking-[0.35em] uppercase mb-6 font-inter font-black">
              03 / Mechanism
            </p>
            <h2
              ...
              {MECHANISM_STEPS.map((step) => (
                <div key={step.num} className="bg-[#0a0a0a] p-6 md:p-8">
                  <div className="flex items-center gap-3 mb-5">
                    <span
                      className="font-serif-display text-white/20 leading-none"
                  ...
                  <h3
                    className="font-black text-white leading-snug mb-4 whitespace-pre-line"
            ...
            {marketStats.map((stat) => (
              <div key={stat.label} className="bg-white px-5 py-6 md:px-7 md:py-7">
                <div className="flex items-start gap-2 mb-3">
                  ...
                  {stat.isEstimate && (
                    <span className="text-[9px] text-black/30 border border-black/15 rounded px-1.5 py-0.5 flex-shrink-0 tracking-wide">
                      推計
                    </span>
                  )}
                </div>
                <p
                  className="font-black text-[#0a0a0a] leading-none mb-2"
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-px bg-black/8 border border-black/8">
                {regionStats.map((r) => (
                  <div key={r.name} className="bg-white px-5 py-5 md:px-6 md:py-6">
                    ...
                    >
                      {r.offices}
                      <span className="text-[13px] font-bold ml-1">事務所</span>
                    </p>
                    <p className="text-[11px] text-black/40">
                      累計 {r.contracts.toLocaleString()} 件
                    </p>
                  </div>
                ))}
              </div>
                ...
                <table className="min-w-full border-collapse text-[13px]">
                  <thead>
                    <tr className="border-b border-black/8">
                      <th className="text-left py-3 px-6 md:px-4 text-[10px] text-black/35 font-inter tracking-[0.15em] uppercase font-medium w-full">
                        事務所名
                      </th>
                      <th className="text-right py-3 px-4 text-[10px] text-black/35 font-inter tracking-[0.15em] uppercase font-medium whitespace-nowrap">
                        エリア
                      </th>
                      <th className="text-right py-3 px-4 text-[10px] text-black/35 font-inter tracking-[0.15em] uppercase font-medium whitespace-nowrap">
                        成約件数
                      </th>
                      <th className="text-right py-3 pl-4 pr-6 md:pr-4 text-[10px] text-black/35 font-inter tracking-[0.15em] uppercase font-medium whitespace-nowrap">
                        月間顧問料計
                      </th>
                    </tr>
                  </thead>
                      ...
                      <tr key={c.name} className="hover:bg-black/[0.02] transition-colors">
                        <td className="py-3.5 px-6 md:px-4">
                          <div className="flex items-center gap-2.5">
                            <span className="text-[10px] text-black/25 font-inter w-4 flex-shrink-0">
                              {String(i + 1).padStart(2, "0")}
                            </span>
                            <span className="text-[#0a0a0a] font-medium">{c.name}</span>
                          </div>
                        </td>
                        <td className="py-3.5 px-4 text-right text-black/50 whitespace-nowrap">
                          {c.area}
                        </td>
                        <td className="py-3.5 px-4 text-right font-medium tabular-nums whitespace-nowrap">
                          {c.contracts} 件
                        </td>
                        <td className="py-3.5 pl-4 pr-6 md:pr-4 text-right font-bold tabular-nums whitespace-nowrap">
                          ¥{(c.monthlyFee / 10000).toFixed(1)}万
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          <div className="max-w-5xl mx-auto flex flex-col md:flex-row md:items-end md:justify-between gap-10">
            ...
            <Link
              ...
              className="
                ...
                px-7 py-4 text-[13px] font-bold tracking-[0.18em] uppercase
